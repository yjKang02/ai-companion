...
v1에는 `ROOM_REGISTRY`·library_id·상대 경로·삭제 작업이 없다. room_id는 애플리케이션이 방 존재를 검사하는 논리 참조이며 registry FK로 강제되지 않는다. 위 전체 ERD와 다른 구현 경계다. 영구 방 조립에는 아래 v2 확장이 필요하며 여러 라이브러리를 하나의 내부 DB에 합쳐 사용하지 않는다.

`create()`는 v1을 생성하고 `open()`은 v1/v2를 지원한다. DB 생성 실패의 부분 파일이나 미지원 버전을 자동 초기화하지 않는다. 기본 경로·외부 프로파일 형식·비밀 저장 방식은 이 어댑터의 선택 사항이 아니다.

### 내부 SQLite v2와 단일 라이브러리 등록부

[등록부 어댑터](../src/ai_companion/adapters/room_registry.py)는 v1 테이블을 유지하고 다음 테이블을 추가한다. `migrate_runtime_v2()`는 실행자를 종료한 상태에서 새 SQLite 백업을 검증한 뒤 트랜잭션으로 스키마를 확장한다. 기존 백업을 덮어쓰지 않으며 자동 이전·자동 복원·라이브러리 가져오기는 제공하지 않는다.

| 실제 테이블 | 원본과 관계 |
| --- | --- |
| `library` | 단일 행의 라이브러리 UUID. 외부 `.library-id`와 대조 |
| `room_registry` | room_id PK, 파일 storage_id, creating/active/deleting/cleaning 단계, 정리 폴더 device/inode. 본문 없음 |
| `retired_rooms` | 삭제 완료한 room_id만 보존. 재사용과 지연 결과의 방 부활 방지. 본문·설정·외부 ID 없음 |

v2도 단일 라이브러리이므로 `(library_id, room_id)` 복합 키나 별도 `CLEANUP_JOB` 테이블은 만들지 않는다. 정리 단계와 폴더 식별자를 registry 행에 보존하고 정리가 끝나야 행을 제거한다. 파일 경로는 DB의 임의 문자열 대신 검증된 UUID에서 계산한다. 기존 바인딩·접수표는 보존하지만 registry FK를 추가하지 않고 애플리케이션의 논리 참조 검사를 유지한다. 이전 v1의 room_id를 임의 외부 파일과 자동 연결하지 않는다.

현재 `library_path`는 방 폴더들의 루트다. 예를 들어 `library_path=data/rooms`이면 `.library-id`·`.library-lock`과 `<room_id>/conversation.db`가 그 안에 놓인다. 생성/삭제 중에는 `.creating-<room_id>`·`.deleting-<room_id>` 폴더를 사용한다. 내부 DB와 그 잠금/백업은 이 루트 밖에 둔다. DB 보조 파일 외 미확인 파일·하위 폴더는 자동 삭제하지 않는다. 페르소나·에셋의 상세 배치는 해당 구현에서 별도로 정한다.

삭제 성공 시 방 DB·바인딩·접수표·registry는 제거되지만 삭제 UUID와 별도 백업은 남는다. 백업이나 외부 메신저 기록의 삭제까지 뜻하지 않는다. 실행 중 링크·외부 파일 교체를 지원하지 않으며, 조립 계층의 잠금·복구·실패 제한은 [영구 방 수명주기 계약](백엔드구현설계.md#영구-방-수명주기-조립-계약)을 따른다.
...
[SqliteRoomDatabase](../src/ai_companion/adapters/sqlite_room.py)는 외부 파일 하나의 저장을 담당한다. 내부 DB와 application_id를 구분하고 독립된 user_version=1을 사용한다. 내부 DB의 v2 이전은 방 파일의 버전을 바꾸지 않는다. 폴더 경로·생성 게시·파일 삭제는 이 구성요소 대신 영구 조립 계층이 담당한다.
...
`open()`은 방 ID·DB 종류·버전·무결성·누락된 턴/응답을 검사하며 상태를 바꾸지 않는다. 시작 시 소유 잠금을 확보한 `persistent_room_store()`가 `interrupt_pending()`을 호출하면 미완료 턴만 `interrupted`로 바뀐다. 완료 이력은 유지하고 중복 입력으로 중단 턴을 재실행하지 않는다. 파일 구성요소와 방 삭제·여러 DB 복구 조립의 [구현 경계](백엔드구현설계.md#방별-sqlite-파일-구현-경계)를 구분한다.

### 저장·복구·삭제의 구현 계약 초안

1. 방 생성은 creating 등록 후 고유 ID의 임시 폴더에서 DB를 완성하고 게시한다. DB가 완성된 뒤 active 등록 전 중단되면 다음 시작에서 검증해 게시·등록을 완료한다. 같은 ID·다른 데이터는 덮어쓰지 않는다. 누락·미완성·손상 DB는 자동 복원하지 않고 시작을 중단한다.
...
5. 삭제는 내부 registry에 deleting을 기록한 뒤 실행·입력을 차단하고 대상을 검증·격리한다. 현재 v2는 별도 CLEANUP_JOB 대신 registry의 cleaning 단계에 정리 작업을 보존한다. 방 DB/보조 파일을 정리한 후 내부 방별 receipt/바인딩·registry를 제거하고 삭제 UUID만 남긴다. 공용 연결·프로파일·다른 방·공용 수신 커서는 보존한다. 소유 에셋 삭제는 후속 범위이며, 경로와 방 ID를 검증하고 라이브러리 전체를 삭제 대상으로 삼지 않는다.
