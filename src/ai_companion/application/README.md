...
방 생성·편집에는 연결 ID 없는 `RoomModelConfig`를 사용한다. `RoomBindings`를 별도 주입하며 `bind()`로 실행 연결을 지정·해제한다. `list_rooms()`·`get()`·`history()`는 모델 연결 없이 조회할 수 있고 `binding()`은 로컬 연결 상태를 조회한다. 콘텐츠 revision과 바인딩 generation은 독립적이다. 생성된 미연결 방은 실행 검증이 완료된 상태가 아니다.

영구 저장은 [조립 지점](../bootstrap.py)의 `persistent_room_backend()`를 사용한다. 파일 소유 잠금과 시작 복구를 마친 뒤 같은 `RoomService`를 제공한다. 호출자는 컨텍스트 종료 전에 진행 중인 요청을 완료하거나 취소 후 합류해야 한다. 라이브러리 초기화·DB 이전·파일 삭제 규칙은 애플리케이션 계층이 아닌 어댑터가 담당한다.
