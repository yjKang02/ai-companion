...
독립된 대화방에서 캐릭터와 대화하고 방별 모델·페르소나·메신저를 설정하는 개인용 AI Companion을 개발한다. 채택한 구현 방향은 [로컬 서버 + 로컬 웹 UI](docs/결정/002-로컬서버-웹UI.md)다. 현재 CLI는 기존 Python 기반 Discord·LM Studio 경로로 실행된다. 새 방 백엔드와 내부 운영 SQLite 어댑터는 추가했지만 CLI 전환·방 대화의 영구 저장·웹 서버·웹 UI는 아직 제공하지 않는다. 단계별 상태의 원본은 [작업 목록](docs/프로젝트.md#작업-목록)이다.
...
[내부 SQLite 어댑터](src/ai_companion/adapters/sqlite_runtime.py)는 모델 연결·방 바인딩·입력 접수표를 파일에 보존한다. 방 본문·대화 이력은 아직 메모리이므로 전체 재시작 복구가 되는 것은 아니다. 개발용 주입 방법과 제한은 [내부 SQLite 계약](docs/백엔드구현설계.md#내부-운영-sqlite-어댑터)을 따른다. 기본 CLI에는 자동 연결하지 않는다.
...
기존 CLI의 SQLite 전환, 웹 관리 화면, 페르소나 실시간 편집, 개인 사실 저장·삭제, 자동 기억, 외부 API 어댑터는 후속 작업이다. 현재 CLI의 페르소나는 `.env`의 시스템 지시이며 변경 후 재시작해야 한다. 이 버전은 전체 제품 MVP 이전의 연결 기반이다.
...
[현재 기초 설계](docs/기초설계.md) · [UI 설계](docs/UI설계.md) · [작업 목록](docs/프로젝트.md#작업-목록) · [작업 규칙](AGENTS.md)
