...
독립된 대화방에서 캐릭터와 대화하고 방별 모델·페르소나·메신저를 설정하는 개인용 AI Companion을 개발한다. 구현 방식은 [로컬 서버 + 로컬 웹 UI](docs/결정/002-로컬서버-웹UI.md)다. 현재 실행 코드는 Python 기반 Discord·LM Studio 연결이며 새 방 시스템과 웹 UI는 아직 구현하지 않았다.
