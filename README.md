...
코어 의존성 테스트는 정적 import를 표준 라이브러리와 `domain`·`application` 내부로 제한한다. 상대 import를 해석하고 루트 패키지 초기화 파일과 application 하위 폴더도 검사해 어댑터·조립 계층 의존성을 차단한다. 동적 import는 이 정적 검사의 대상이 아니다.
